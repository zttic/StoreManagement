    ...
    <n-space vertical>
        <n-form ref="formRef" :model="formValue" :rules="rules" label-placement="left" label-width="auto">
            ...
            <n-form-item label="封面图片" path="image.cover">
                <n-upload :custom-request="uploadImage" @remove="removeImage($event, 'cover')" list-type="image-card"
                    max="1" :action="uploadAction" data="cover" v-model:file-list="coverInstance">
                </n-upload>
            </n-form-item>
...
import { ref, h, onMounted, watch, defineProps, type Ref } from "vue"
...
watch(() => props.initialData, (newValue) => {
    ...
    if (newValue !== undefined && Object.keys(newValue).length > 0) {
        formValue.value = {
            product: {
                category: category.value.find(category => category.value === newValue.categoryId)?.label || '',
                name: newValue.name || '',
                unit: unit.value.find(unit => unit.value === Number(newValue.unit))?.label || '',
                isSale: newValue.isSale !== undefined ? newValue.isSale : true,
            },
            image: {
                cover: newValue.imgurl[0]['original_url'] || [],
                other: []
            },
        };
        item.value = newValue.item

        for (let i = 0; i < newValue.gallery.length; i++) {
            otherInstance.value.push({ name: newValue.gallery[i]['original_url'], url: newValue.gallery[i]['presigned_url'], id: '1', status: 'finished' })
            formValue.value.image.other.push(newValue.gallery[i]['original_url'])
        }
        coverInstance.value = [{ name: newValue.imgurl[0]['original_url'], url: newValue.imgurl[0]['presigned_url'], id: '1', status: 'finished'}]
...
function addItem(row: any) {
    console.log("check props:", props.initialData)
...
async function editProduct() {
    ...
    try {
        ...
        await axios.put("http://localhost:8000/management/product/", jsonData, {
            headers: {
                "Content-Type": "application/json"
            }
        })
